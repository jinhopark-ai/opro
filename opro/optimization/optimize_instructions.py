...
def main(_):
    ...
    print(f"Optimizer: {optimizer_llm_name}")
    print(f"Dataset: {dataset_name}")
    print(f"Task: {task_name}")
    print(f"Meta prompt type: {meta_prompt_type}")
    """모델과 토크나이저를 로드합니다."""
    # GPU 사용 가능 여부와 개수 확인
    if torch.cuda.is_available():
        num_gpus = torch.cuda.device_count()
        print(f"[DEBUG] Using {num_gpus} GPUs")
        for i in range(num_gpus):
            print(f"[DEBUG] GPU {i}: {torch.cuda.get_device_name(i)}")
    else:
        print("[DEBUG] No GPU available, using CPU")
    ...
    if optimizer_llm_name.lower() == "text-bison":
        ...
        optimizer_finetuned_palm_num_decodes = 2
        ...
        optimizer_finetuned_open_llm_batch_size = 8
        optimizer_finetuned_open_llm_num_servers = 1
        optimizer_finetuned_open_llm_dict = dict()
        optimizer_finetuned_open_llm_dict["temperature"] = (
            optimizer_finetuned_open_llm_temperature
        )
        optimizer_finetuned_open_llm_dict["batch_size"] = (
            optimizer_finetuned_open_llm_batch_size
        )
        optimizer_finetuned_open_llm_dict["num_servers"] = (
            optimizer_finetuned_open_llm_num_servers
        )
        optimizer_finetuned_open_llm_dict["max_decode_steps"] = (
            optimizer_finetuned_open_llm_max_decode_steps
        )

        print(f"[DEBUG] Optimizer model parameters:")
        print(f"Temperature: {optimizer_finetuned_open_llm_dict['temperature']}")
        ...
        optimizer_llm_dict.update(optimizer_finetuned_open_llm_dict)
        call_optimizer_server_func = call_optimizer_local_server_func
